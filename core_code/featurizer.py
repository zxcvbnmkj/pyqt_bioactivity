...
class DownstreamTransformFn(object):
    ...
    def __call__(self, raw_data):
        ...
        Gen 根据原始数据特征并返回单个图形数据。参数： raw_data：它包含微笑和标签，我们通过rdkit将微笑转换为mol，然后将mol转换为图形数据。返回：数据：它包含重塑标签和微笑
        """
        self.i+=1
