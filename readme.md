...
- [x] 已添加对 cuda 的支持。注意，若想要使用 gpu 训练，需要下载 cuda 版本的 pytorch。直接安装 `requirements.txt` 得到的是 cpu 版本的 pytorch 。
...
- [x] 用户自主命名存储模型名字、处理结果文件，而不是默认为 `best_model.pth` 等
- [x] 添加对 cuda 的支持
- [ ] 对三维提取的测试
- [x] 补全 `requirements.txt` 中所需库，或者换一种环境管理方式（如 PDM ）
...
# 药物分子活性预测 PYQT 程序 V1.0 使用手册

见：基于多模态信息融合的药物分子活性预测系统v1.0说明书.docx